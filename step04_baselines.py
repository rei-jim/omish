...
for name, X, tune in [("chance", clin, False)] + [(n, X, True) for n, X in feature_sets.items()]:
    ...
    for r in range(REPEATS):
        ...
        per_rep.append((roc_auc_score(y, oof), average_precision_score(y, oof), f1_score(y, oof > 0.5)))
        oofs[f"{name}|{r}"] = oof
    per_rep = np.array(per_rep)
    rows.append([name, *per_rep.mean(0), *np.percentile(per_rep[:, 0], [2.5, 97.5])])
res = pd.DataFrame(rows, columns=["model", "ROC-AUC", "PR-AUC", "F1", "AUC 2.5%", "AUC 97.5%"]).set_index("model")
...
lines = [f"{REPEATS} repeats x {FOLDS}-fold stratified CV, C tuned by inner {FOLDS}-fold CV (nested). Prevalence {y.mean():.3f}.",
